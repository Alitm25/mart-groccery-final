...
export function MiniProductSlider({}) {
    const linkHandler = (index :number) => {
        if (index === 0) {
            return 12;
        } else if (index === 1) {
            return 6;
                        ...
                        <SwiperSlide key={index}>
                            <div style={{backgroundImage: `url(${item.image})`, backgroundColor: item.color}} className="flex items-center aspect-slider pl-4 lg:pl-12 rounded-[10px] bg-[length:50%] bg-no-repeat bg-right-bottom">
                                <div className="basis-1/2 flex flex-col justify-between">
                                    ...
                                    <Link href={`/single-category/${linkHandler(index)}`} className="mt-6 pl-[15px] pr-2.5 py-2 bg-green-200 hover:bg-yellow-100 transition-[background-color] rounded-[3px] cursor-pointer inline-flex max-w-max items-center gap-2.5">
